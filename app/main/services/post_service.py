...
def get_user_posts(username):
    ...
    posts = db.session.query(Post.public_id, Post.content, Post.posted_on, Post.post_author, User.first_name, User.last_name).filter(User.public_id==user_id).filter(user_post_rel.c.user_id==User.public_id).filter(user_post_rel.c.public_id==Post.public_id).all()
